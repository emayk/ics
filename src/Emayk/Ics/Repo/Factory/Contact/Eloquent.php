...
class Eloquent extends BaseModel
{
	...
	protected $hidden =['parent_id','parent_type','uuid'];
	...
	public function getContactWithoutMorp($id, $type)
	{
		return $this->OfParentId($id)->OfParentType($type);
	}

	public function getContactFromFactorySupplier($idSupplier)
	{
		return $this->OfParentId($idSupplier)->OfParentType('\Emayk\Ics\Repo\Suppliers\Suppliers');
	}

	public function scopeOfParentId($q, $id)
	{
		return $q->whereParentId($id);
	}

	public function scopeOfParentType($q, $type)
	{
		return $q->whereParentType($type);
	}

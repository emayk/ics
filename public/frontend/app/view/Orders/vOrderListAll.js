	...
	columns: [
		...
		{header: 'Config', xtype: 'actioncolumn', width: 40, items: [
			{    itemId: 'edit', iconCls: 'action_edit', tooltip: 'Edit Order',
				handler: function (grid, rowIndex, colIndex, item, e) {
					this.fireEvent('itemclick', this, 'edit', grid, rowIndex, colIndex, item, e);
				}
			},
			{    itemId: 'delete', iconCls: 'action_delete', tooltip: 'Delete Order',
				handler: function (grid, rowIndex, colIndex, item, e) {
					this.fireEvent('itemclick', this, 'delete', grid, rowIndex, colIndex, item, e);
				}
			},
		...
		{header: 'Status', dataIndex: 'status', width: 35,
			renderer: function (value, metadata, record, rowIndex, colIndex, store) {
				if (value == 1) {
					metadata.css = 'status_open';
				} else {
					metadata.css = 'status_close';
				}
		...
		{header: 'Approve', dataIndex: 'approve_id', width: 30,
			renderer: function (value, metadata, record, rowIndex, colIndex, store) {
				if (value == 1) {
					metadata.css = 'approve_yes';
				} else {
					metadata.css = 'approve_no';
				}
				metadata.attr = 'ext:qtip="' + (value) + '"';
				return '';
			}

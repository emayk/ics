...
// Configuration
		...
		texts: {    status: ['QUEUED', 'UPLOADING', 'UNKNOWN', 'FAILED', 'DONE'],
...
// Hack: loaded of the actual file (plupload is sometimes a step ahead)
		...
		PluploadError: function (uploader, data) {
			data.file.status = 4;
			if (data.code == -600) {
				data.file.msg = String.format('{0}', this.texts.statusInvalidSizeText);
			}
			else if (data.code == -700) {
				data.file.msg = String.format('{0}', this.texts.statusInvalidExtensionText);
			}
			else {
				data.file.msg = String.format('{2} ({0}: {1})', data.code, data.details, data.message);
			}

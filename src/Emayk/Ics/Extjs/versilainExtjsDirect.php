...
*
* Diambil dari :
* https://sitefront.googlecode.com/svn-history/r284/trunk/front-framework/lib/ExtDirect.php
*
**/
...
class ExtDirect
{
     ...
     */
    static public $namespace = 'FrontSuite.server';

    /**
     * @var string   Ext.Direct API attribute "descriptor"
     */
    static public $descriptor = 'FrontSuite.server.REMOTING_API';
...
JAVASCRIPT;
        ...
        if ($trace) trace_exit(strtr( $template, $elements ));
...
class ExtDirectRequest
{
    ...
    {
        global $trace, $debug, $options;
        if ($trace) trace_entry();
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            $input = file_get_contents( 'php://input' );

            $request = json_decode(str_replace('\\"', '"', $input));
            if ($debug) {
                debug_print('POSTDATA');
                debug_print(print_r($request,true));
            }
        ...
        elseif ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_REQUEST['action']) && isset($options) && isset($options['test_mode']) && $options['test_mode']) {
            header('Content-type: text/plain');
            # action: "Database"
            # data: [{dbtype:sqlite, db_name:foo, db_label:foo}]
            ...
            if (!isset($_REQUEST['action']) ||
                ...
                !isset($_REQUEST['data']) ||
                !isset($_REQUEST['tid'])) {
                echo "The 'action' (the PHP class), 'method', 'tid' (transaction id or method number), and 'data' (JSON-encoded function parameters. e.g. '[{dbtype:sqlite, db_name:foo, db_label:foo}]') must all be supplied to execute the Ext Direct layer with a GET method in test_mode\n";
                exit(0);
            }
            ...
            if ($debug) {
                ...
                debug_print(print_r($data,true));
            }
            if (!$data) throw new Exception('Invalid JSON data for the "data" parameter: ' . $_REQUEST['data']);
            $tid    = $_REQUEST['tid'];
            $this->actions[] = new ExtDirectAction( $action, $method, $data, $tid, false, false );
        }
        if ($trace) trace_exit();
...
class ExtDirectAction
{
    ...
    public function run()
    {
        ...
        catch ( Exception $e )
        {
            ...
            if ( ExtDirect::$debug || (isset($options) && isset($options['test_mode']) && $options['test_mode']) )
                $response = array(
                    'type'    => 'exception',
                    'tid'     => $this->transaction_id,
    ...
    protected function call_action()
    {
        ...
        $result = $this->call_action_func_array( array( $this->instance, $this->method ), $parameters );
        if ($trace) trace_exit($result);
        return($result);
    }

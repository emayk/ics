...
class ProductsuppliersEloquent implements ProductsuppliersInterface{
    ...
    **/
    ...
    public function all()
    {
        ...
        $productsuppliers = $this->productsuppliers
            ->orderBy('id','DESC')
            ->skip($start)
            ->take($limit)
            ->get()->toArray();
        $total = $this->productsuppliers
            ->all()->count();

        $productsupplierss = array(
            'success' => true,
            'results' => $productsuppliers,
            'total' => $total
        );

        return Response::json($productsupplierss)

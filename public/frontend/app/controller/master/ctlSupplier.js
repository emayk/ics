...
onChangeTypeChart: function(item, e, opt){
	...
	if (item.itemId == 'pie') {panel.getLayout().setActiveItem(0); }
	if (item.itemId == 'column') {panel.getLayout().setActiveItem(1); }
},
onDownloChartAs : function(item,e,opt){
	...
	if (item.itemId == 'png') {
		Ext.MessageBox.confirm('Confirm Download', 'Apakah Anda Ingin Download sebagai Image',function(choice){
			if (choice == 'yes' ) { chart.save({ type: 'image/png' } )};
		});
	}
		if (item.itemId == 'svg') {
		Ext.MessageBox.confirm('Confirm Download', 'Apakah Anda Ingin Download sebagai SVG',function(choice){
			if (choice == 'yes' ) { chart.save({ type: 'image/svg+xml' } )};
		});
	}

	if (item.itemId == 'pdf') { msgError('Belum Implementasi, Next Feature ')};
...
**/
	...
	on_formPhone_btn_save: function(btn){
		...
		log(val);
		if (!form.isValid()){ msgError('Form Invalid'); return false ;}
				/*cek id */
				var method, url = getApiUrl() + '/supplier_phones';
				if (val.id.length > 0 ){
						method = 'PUT'; url = url + '/'+val.id;
				}else{
					method = 'POST'; url = url;
				};
				form.submit({
	...
	on_grid_phone_btn_add: function(btn){
		...
		if (!selectedSupplier) { log('Supplier belum dipilih'); return false; }
		var selectedOffice = me.getGridOffices().getSelectionModel().getSelection()[0] ;
		if (!selectedOffice) {log('Office Supplier not yet selected'); return false;}
	...
	on_grid_phone_btn_remove: function(btn){
		...
		if (record === undefined) {log(' Phone Office Supplier Not Selected' );
...
**/
	...
	btn_save_form_office: function(btn){
		...
		if (form.isValid()){
				if (val.id.length > 0 ){ method = 'PUT'; url = url + '/'+rec.get('id');}else { method = 'POST'; url = url };
				form.submit({
					waitMsg : 'Please Wait....',
					url : url, method : method,
					params : { _token : gettoken(), uid : getIdLogin() },
	...
	btn_add_Office_Supplier:  function(btn){
			...
			if (!record) { log('Supplier belum dipilih'); return false; }
	...
	btn_remove_Office_Supplier:  function(btn){
		...
		if (record === undefined) {log(' Supplier Office Not Selected' );
...
**/
	...
	var me = this, record = me.getGridSupplier().getSelectionModel().getSelection()[0] ;
		if (record === undefined) {log(' Supplier Not Selected' );
	...
	onBtnsaveSupplierClick: function(btn){
		...
		val = form.getValues();
		if (!form.isValid()) {msgError('Please Input Form Correctly'); return; };
		if (val.province_id.length == 0) {msgError('Please Select Province');return; }
		if (val.city_id.length == 0) {msgError('Please Select City');return;}
	 ...
	 if (val.id.length > 0){
	 	...
	 	msg = 'Updated ';
	 }else {method = 'POST'; url = getApiUrl() +'/supplier'; msg = 'Added '; };
	...
	onBtnHelpSupplierClick: function(btn){ winHelp('FormAddSupplier');},
	onBtnCancelSupplierClick: function(btn){	/*this.getTabSupplier().remove( this.getFormSupplier() );*/ btn.up('form').close(); },
	...
	onComboCountriesChange : function(old,newVal){log('Change cbCountries New value' + newVal); },
	...
	onGridSupplier_Edit : function(btn){
			...
			if (!record) { log('Cannot found record'); return false; }
			cDir(record);
...
**/
	...
	onClick_Contact_btnRemove : function(btn){
		...
		if (record === undefined) {log(' Contact Person Tidak dipilih' ); return false; }
		var storeContacts = me.getGridContacts().getStore();
		/*remove from store*/
		storeContacts.remove(record); storeContacts.sync();
	},
	onAddContact_addBtn_click : function(btn){
		var me = this, record = me.getSelectionGridSupplier();

		log('Load Departement Store');
		if (record === undefined) {log('Supplier tidak ada yg dipilih'); return false; }
	...
	on_BtnHelp_FormContact : function(btn){ winHelp('AddContact')},
	on_BtnCancel_FormContact : function(btn){btn.up('form').close(); /*button cancel at form contact*/ },
...
**/
	...
	onButton_Save_FormAccountBankClick : function(btn){
		...
		if (!form.isValid()) {msgError('Please Correctly '); return false; }
	...
	onButton_remove_AccountClick : function(btn){
		...
		if (record === undefined) {log(' Account Bank Not Selected' );
	...
	onButton_Add_AccountClick : function(btn){
		...
		if (record === undefined) {log('tidak ada yg dipilih'); return false; }
...
**/
    ...
    **/
	...
	grid_Disabled : function(grid){	grid.setDisabled(true); },
	grid_Enabled : function(grid){	grid.setDisabled(false); },
